...
//         card.show(menuP, "SP");
   ...
   /*
    * ~입실~ 좌석 번호 가져와서 -> if(sit=0) if(room=0) -> 좌석을 선택해주세요 else 카드 쇼 ~시간연장 버튼??~
    * 
    * ~좌석 이동~ -> 회원 업데이트 활용? 이동하실 좌석을 선택해주세요(밑에 조그맣게 라벨 - 현재 좌석:N번) -> 좌석을(sitnum)
    * 선택함 -> sitnum번 좌석으로 변경하시겠습니까? ->변경되었습니다
    * 
    * 디비에서 좌석 번호 가져온다 / 이 좌석 번호를 JLabel로 "현재 좌석:dto.getSit" 으로 보여준다 -> 버튼을 눌러서 좌석을
    * 선택한다 (좌석의 버튼 넘버를 받아온다) -> 창을 띄워서 sitnum으로 변경하시겠습니까? -> Y: 변경되었습니다 창 + (좌석 버튼이
    * db로 가서 sit 데이터 변경함) N: 취소되었습니다 창 - 메인P로
    * 
    * 퇴실 활용? 퇴실은 좌석번호0이고 좌석이동은 선택 좌석 번호 주면 됨
    * 
    */
